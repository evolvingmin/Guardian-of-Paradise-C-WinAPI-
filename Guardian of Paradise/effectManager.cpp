...
void effectManager::addEffect(string effectName, const char* imageName, int imageWidth,
	int imageHeight, int effectWidth, int effectHeight, int* arr, int arrlen, int fps, float elapsedTime, int buffer)
{
	image* img;
	arrEffects vEffectBuffer;	//이펙트 버퍼 벡터
	arrEffect mArrEffect;		//키 값과 이펙트 버퍼 담아줄 맵
	...
	{
		vEffectBuffer.push_back(new effect);
		vEffectBuffer[i]->init(img, effectWidth, effectHeight, arr, arrlen, fps, elapsedTime);
	}
...
void effectManager::play(string effectName, int x, int y)
{
	...
	for (vIter = _vTotalEffects.begin(); vIter != _vTotalEffects.end(); ++vIter)
	{
		...
		for (mIter = vIter->begin(); mIter != vIter->end(); ++mIter)
		{
			//이펙트 이름과 비교해서 같지 않으면 다음 맵으로 넘어감
