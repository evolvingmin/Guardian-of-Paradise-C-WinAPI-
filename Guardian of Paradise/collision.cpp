...
namespace FOX_UTIL
{
	...
	bool isCollisionReaction(gameObject* hold, gameObject* move)
	{
		RECT rcInter;

		IntersectRect(&rcInter, &hold->boundingBox(), &move->boundingBox());
		...
		{
			//위
			if (rcInter.top == hold->boundingBox().top)
			{
				move->setY(move->getY() - interH);
			}
			//아래
			else if (rcInter.bottom == hold->boundingBox().bottom)
			{
				move->setY(move->getY() + interH);
			}
		}
		...
		{
			//좌
			if (rcInter.left == hold->boundingBox().left)
			{
				move->setX(move->getX() - interW);
			}
			//우
			else if (rcInter.right == hold->boundingBox().right)
			{
				move->setX(move->getX() + interW);
			}
		}
